# Snooker Practice Log — v4.13.0.2.1

Built from the validated v4.0.0 ES-module release.

## v4.13.0.2.1 production-safety patch
...
Confirm version: the header should show v4.13.0.2.1.
...
- Bayesian recommendation integration for success-rate drills.
- Bayesian confidence now influences routine ranking through score deltas:
  - high uncertainty increases repeat priority
  - credible interval below target increases stabilization priority
  - credible interval above target lowers repeat priority and signals progression readiness
  - target overlap encourages holding the current target
- Bayesian reasons are added to recommendation explanations.
- No storage, IndexedDB, render orchestration, or session lifecycle changes.
