...
function smartBuilderEtuContextSafe(logs=data.logs || []) {
  try {
    ...
    return {load, domainLoads, latestSession, acute, baseline, ratio, sustainable, undertrained, overloaded, state, label, guidance};
  } catch (err) {
    try { logAppError(err, "smartBuilderEtuContextSafe"); } catch (_) {}
    return {load:null, domainLoads:[], latestSession:null, acute:0, baseline:0, ratio:1, sustainable:0, undertrained:[], overloaded:[], state:"unknown", label:"ETU unavailable", guidance:"use normal builder logic"};
  }
}
function smartBuilderRoutineDomainKeySafe(routine) {
  try {
    const map = getRoutineSkillMap(routine);
    const raw = map?.primarySkill || routine?.primarySkill || routine?.skill || routine?.category || routine?.folder || routine?.name || "general";
    return predictionDomainKeyFromLogSafe({category:raw, routineName:routine?.name});
  } catch (_) {
    return "general";
  }
}
function smartBuilderEtuAdjustmentSafe(state, etuContext) {
  try {
    const ctx = etuContext || {};
    const energy = routineEnergyProfile(state);
    const phase = String(state?.phase || "");
    const domainKey = smartBuilderRoutineDomainKeySafe(state?.routine || {});
    const undertrainedKeys = new Set((ctx.undertrained || []).map(d => String(d.key || "")));
    const overloadedKeys = new Set((ctx.overloaded || []).map(d => String(d.key || "")));
    let adj = 0;
    const reasons = [];
    if (undertrainedKeys.has(domainKey)) {
      adj += ctx.state === "high" ? 2.5 : 7;
      reasons.push(`ETU undertrained domain: ${predictionDomainLabelSafe(domainKey)}`);
    }
    if (ctx.state === "high" || ctx.state === "moderate_high") {
      if (Number(energy.fatigue || 0) >= 4 || Number(energy.cognitive || 0) >= 4) {
        adj -= ctx.state === "high" ? 8 : 4;
        reasons.push("ETU load guard: reduce high-energy drill stacking");
      }
      if (["recover","maintain","stabilize"].includes(phase)) {
        adj += ctx.state === "high" ? 6 : 3;
        reasons.push("ETU load guard: favors consolidation/recovery");
      }
      if (state?.blockType === "pressure" && ctx.state === "high") {
        adj -= 5;
        reasons.push("ETU load guard: pressure capped after high load");
      }
    } else if (ctx.state === "low") {
      if (["progress","baseline"].includes(phase) || state?.blockType === "primary") {
        adj += 5;
        reasons.push("ETU load low: acquisition capacity available");
      }
    }
    if (overloadedKeys.has(domainKey) && ctx.state === "high") {
      adj -= 3;
      reasons.push(`ETU balance: ${predictionDomainLabelSafe(domainKey)} already heavily loaded`);
    }
    return {adjustment:adj, reasons};
  } catch (err) {
    try { logAppError(err, "smartBuilderEtuAdjustmentSafe"); } catch (_) {}
...
function renderSmartBuilderEtuContextSafe(ctx) {
  try {
    ...
    return `<div class="adaptive-rationale"><strong>ETU-aware builder:</strong> ${escapeHtml(ctx.label)} · 7d ${numText(ctx.acute)} ETU vs baseline ${numText(ctx.baseline)} (${numText(ratio)}×) · latest ${numText(latest)} ETU · undertrained: ${escapeHtml(under)}. ${escapeHtml(ctx.guidance)}</div>`;
  } catch (_) {
...
function adaptiveSessionStructure(goal, duration, strictness, periodization = {}) {
  ...
  const ranked = states.map(s => {
    ...
    boost += Number(etuAdjustment.adjustment || 0);
    ...
    return {...s, adaptiveScore: smartScore, transferValue, energyProfile, blockType:blockTypeForState(s, goal), recommendationProfile:profile, etuReasons:(etuAdjustment.reasons || []), reasons:[...(s.reasons || []), `transfer value ${transferValue}/100`, `energy load C${energyProfile.cognitive}/F${energyProfile.fatigue}/Conf${energyProfile.confidence}`, ...(etuAdjustment.reasons || []), buildContextAwareReason(profile || {contextualFit:contextualFitForRoutine(s.routine, {logs:s.logs||[], vals:(s.logs||[]).map(l=>Number(l.normalizedScore||0)), hit:s.hit})}), ...(profile?.reasons || []).slice(0,3)]};
  ...
  return {effectiveGoal, targetMinutes, estimatedMinutes, horizonWeeks, daysToCompetition, globalReasons, blocks, routineIds, ranked, budgets, budgetUsage, etuContext};
}
...
function estimateEffectiveTrainingLoadSafe(logs) {
  ...
  const rows = Array.from(sessions.values()).map(arr => {
    ...
    const pressureShare = arr.filter(log => {
      const t = textForLog(log);
      return log?.pressureEnabled || log?.pressureMode || t.includes("pressure") || t.includes("match") || t.includes("tactical") || t.includes("safety") || t.includes("decider");
    }).length / Math.max(1, arr.length);
    const transferShare = arr.filter(log => {
      const t = textForLog(log);
      return t.includes("transfer") || t.includes("break") || t.includes("line-up") || t.includes("lineup") || t.includes("cue ball") || t.includes("cue-ball") || t.includes("long pot");
    }).length / Math.max(1, arr.length);
    const adaptiveShare = arr.filter(log => {
      const t = textForLog(log);
      return log?.recommendationId || log?.recommendationAccepted || log?.source === "smart" || log?.sessionType === "adaptive" || log?.planType === "adaptive" || t.includes("adaptive") || t.includes("smart");
    }).length / Math.max(1, arr.length);
    ...
    const avgQuality = qualityVals.length ? avg(qualityVals) : null;
    const avgFatigue = fatigueVals.length ? avg(fatigueVals) : null;

    // Raw ETU is intentionally simple: approximate unweighted table-time exposure.
    // Effective ETU is the calibrated development load used by predictions.
    const rawEtu = minutes > 0 ? Math.max(0.08, minutes / 45) : Math.max(0.10, arr.length * 0.25);
    let durationComponent;
    if (minutes > 0) {
      if (minutes <= 45) durationComponent = Math.max(0.20, minutes / 45);
      else if (minutes <= 90) durationComponent = 1 + ((minutes - 45) / 45) * 0.75;
      else durationComponent = 1.75 + Math.sqrt(Math.max(0, minutes - 90) / 45) * 0.55;
    } else {
      durationComponent = Math.sqrt(Math.max(1, arr.length) / 4);
    }
    durationComponent = Math.max(0.18, Math.min(2.55, durationComponent));

    const diversityFactor = 1 + Math.min(0.60, Math.max(0, uniqueRoutines - 1) * 0.075);
    const densityFactor = 1 + Math.min(0.25, Math.max(0, arr.length - 1) * 0.028);
    const pressureFactor = 1 + Math.min(0.18, pressureShare * 0.18);
    const transferFactor = 1 + Math.min(0.16, transferShare * 0.16);
    const adaptiveFactor = 1 + Math.min(0.16, adaptiveShare * 0.16);
    const challengeFactor = hitRate >= 35 && hitRate <= 70 ? 1.10 : hitRate >= 20 && hitRate < 35 ? 1.02 : hitRate > 70 && hitRate <= 85 ? 1.03 : hitRate < 12 || hitRate > 92 ? 0.86 : 0.96;
    const qualityFactor = qualityVals.length ? Math.max(0.80, Math.min(1.16, avg(qualityVals) / 3.35)) : 1.0;
  ...
  const totalEtu = rows.reduce((sum, row) => sum + row.effectiveEtu, 0);
  const totalRawEtu = rows.reduce((sum, row) => sum + row.rawEtu, 0);
  ...
  const rawEtuPerWeek = totalRawEtu > 0 ? totalRawEtu / Math.max(1, daySpan / 7) : 0;
  const sortedRows = rows.slice().sort((a,b)=>(Number(a.dateValue)||0)-(Number(b.dateValue)||0));
  const componentAvg = key => rows.length ? rows.reduce((sum, row) => sum + Number(row.components?.[key] || 0), 0) / rows.length : 0;
  return {
    totalEtu,
    totalRawEtu,
    totalMinutes,
    ...
    avgRawEtuPerSession: totalRawEtu / sessionCount,
    ...
    componentAverages: {
      duration: componentAvg("duration"),
      diversity: componentAvg("diversity"),
      density: componentAvg("density"),
      pressure: componentAvg("pressure"),
      transfer: componentAvg("transfer"),
      adaptive: componentAvg("adaptive"),
      challenge: componentAvg("challenge"),
...
function predictionDomainKeyFromLogSafe(log) {
  ...
  const text = String(raw || "General").toLowerCase();
  if (text.includes("break")) return "break_building";
  if (text.includes("cue") || text.includes("position") || text.includes("control")) return "cue_ball_control";
  if (text.includes("long")) return "long_potting";
  if (text.includes("safety")) return "safety";
  if (text.includes("pressure") || text.includes("match") || text.includes("decider")) return "pressure";
  if (text.includes("tactical") || text.includes("shot to nothing")) return "tactical";
  if (text.includes("rest")) return "rest_play";
  return String(raw || "general").replace(/[^a-z0-9]+/gi,"_").replace(/^_+|_+$/g,"").toLowerCase() || "general";
}
function predictionDomainLabelSafe(key) {
  const map = {
    break_building:"Break-building",
    cue_ball_control:"Cue-ball Control",
    long_potting:"Long Potting",
    safety:"Safety",
    pressure:"Pressure",
    tactical:"Tactical",
    rest_play:"Rest Play",
  ...
  return map[key] || skillLabel(String(key || "general"));
}
function estimatePredictionDomainEtuSafe(logs) {
  try {
    const scoped = Array.isArray(logs) ? logs : [];
    const sessions = new Map();
    scoped.forEach((log, idx) => {
    ...
    sessions.forEach(arr => {
      const sessionLoad = estimateEffectiveTrainingLoadSafe(arr);
      const sessionEtu = Math.max(0, Number(sessionLoad?.totalEtu || sessionLoad?.avgEtuPerSession || 0));
      const totalMinutes = arr.reduce((sum, l) => sum + Math.max(0, Number(l?.timeMinutes || l?.minutes || 0)), 0);
      arr.forEach(log => {
        const key = predictionDomainKeyFromLogSafe(log);
        if (!domainMap.has(key)) domainMap.set(key, {key, label:predictionDomainLabelSafe(key), etu:0, rawEtu:0, minutes:0, logs:0, hitLogs:0, scoreSum:0, scoreN:0});
        const row = domainMap.get(key);
        const minutes = Math.max(0, Number(log?.timeMinutes || log?.minutes || 0));
        const weight = totalMinutes > 0 ? minutes / totalMinutes : 1 / Math.max(1, arr.length);
        const rawEtu = minutes > 0 ? minutes / 45 : 0.25;
        const score = Number(log?.normalizedScore ?? normalizeScore(log));
        row.etu += sessionEtu * Math.max(0.05, weight);
        row.rawEtu += rawEtu;
        row.minutes += minutes;
        row.logs += 1;
        if (String(log?.performance || "").toLowerCase().includes("target") || String(log?.performance || "").toLowerCase().includes("above")) row.hitLogs += 1;
        if (Number.isFinite(score)) { row.scoreSum += score; row.scoreN += 1; }
      });
    });
    return Array.from(domainMap.values()).map(row => ({
      ...row,
      etu: Math.round(row.etu * 10) / 10,
      rawEtu: Math.round(row.rawEtu * 10) / 10,
      minutes: Math.round(row.minutes),
      avgScore: row.scoreN ? row.scoreSum / row.scoreN : null,
      hitRate: row.logs ? row.hitLogs / row.logs * 100 : null
    })).sort((a,b)=>b.etu-a.etu);
  } catch (err) {
    try { logAppError(err, "estimatePredictionDomainEtuSafe"); } catch (_) {}
    return [];
  }
}
function predictionDomainEtuRequirementSafe(currentLevel, currentScore, nextThreshold) {
  const level = Math.max(1, Math.min(7, Number(currentLevel || 1)));
  const gap = Math.max(0.5, Number(nextThreshold || 0) - Number(currentScore || 0));
  const curve = level <= 2 ? 2.8 : level === 3 ? 5.2 : level === 4 ? 8.5 : 13.0;
  return Math.max(8, Math.round(gap * curve));
}
function predictionDomainForecastabilitySafe(row, domainLoad) {
  const logs = Number(domainLoad?.logs || 0);
  const etu = Number(domainLoad?.etu || 0);
  if (logs < 4 || etu < 3) return {forecastable:false, reason:"needs more domain-specific ETU evidence"};
  if (String(row?.confidence || "").toLowerCase().includes("low")) return {forecastable:true, dampener:0.58, reason:"low domain confidence"};
  if (String(row?.confidence || "").toLowerCase().includes("moderate")) return {forecastable:true, dampener:0.78, reason:"moderate domain confidence"};
  return {forecastable:true, dampener:1.0, reason:"sufficient domain evidence"};
}
function predictionRowsForDomainEtuLedgerSafe(profile, domainLoads) {
  try {
    const byLabel = new Map((domainLoads || []).map(d => [String(d.label || "").toLowerCase(), d]));
    const byKey = new Map((domainLoads || []).map(d => [String(d.key || "").toLowerCase(), d]));
    const rows = (profile?.profile || []).slice().sort((a,b)=>(Number(a.score)||0)-(Number(b.score)||0)).slice(0,7);
    if (!rows.length) return `<div class="analytics-note">Add more logs to build ETU by skill domain.</div>`;
    return rows.map(x => {
      const label = String(x?.label || "Skill");
      const key = predictionDomainKeyFromLogSafe({category:label});
      const load = byLabel.get(label.toLowerCase()) || byKey.get(key) || {etu:0, rawEtu:0, logs:0, minutes:0};
      const score = Number(x?.score || 0);
      const currentLevel = Number(String(x?.level || "L1").replace("L", "")) || 1;
      const nextThreshold = Math.min(98, currentLevel * 14 + 7);
      const required = predictionDomainEtuRequirementSafe(currentLevel, score, nextThreshold);
      const effectiveEtu = Math.max(0, Number(load.etu || 0));
      const rawEtu = Math.max(0, Number(load.rawEtu || 0));
      const progress = Math.max(0, Math.min(100, effectiveEtu / Math.max(1, required) * 100));
      const logs = Math.max(0, Number(load.logs || 0));
      const minutes = Math.max(0, Number(load.minutes || 0));
      const hitRate = Number.isFinite(Number(load.hitRate)) ? `${Math.round(Number(load.hitRate))}% hit` : "hit-rate n/a";
      return `<div class="prediction-domain-etu-row">
        <div class="prediction-domain-etu-main">
    ...
    try { logAppError(err, "predictionRowsForDomainEtuLedgerSafe"); } catch (_) {}
    return `<div class="analytics-note warn">Domain ETU ledger unavailable.</div>`;
  }
}
function renderPredictionCalibrationV2SummarySafe(load, domainLoads, benchmark) {
  ...
  return `<div class="analytics-note"><strong>Prediction calibration v2:</strong> forecasts now separate total ETU from domain-specific ETU, apply a capped sustainable pace (${numText(pace)} ETU/week), and suppress precise calendar windows where benchmark distance or domain evidence is too weak. Largest accumulated domain loads: ${htmlText(top)}. Benchmark guard: ${htmlText(benchmarkGuard)}.</div>`;
}
...
function predictionRowsForDomainsSafe(profile, velocity, confidence, domainLoads=[]) {
  const rows = (profile?.profile || []).slice().sort((a,b)=>(Number(a.score)||0)-(Number(b.score)||0)).slice(0,7);
  ...
  return rows.map(x => {
    const label = String(x?.label || "Skill");
    const key = predictionDomainKeyFromLogSafe({category:label});
    const domainLoad = byLabel.get(label.toLowerCase()) || byKey.get(key) || {etu:0, logs:0};
    const score = Number(x?.score || 0);
    const currentLevel = Number(String(x?.level || "L1").replace("L", "")) || 1;
    const nextThreshold = Math.min(98, currentLevel * 14 + 7);
    const requiredEtu = predictionDomainEtuRequirementSafe(currentLevel, score, nextThreshold);
    const forecastability = predictionDomainForecastabilitySafe(x, domainLoad);
    if (!forecastability.forecastable) {
      return `<div class="context-row prediction-row risk"><span>${htmlText(label)}<br><span class="muted">${htmlText(x?.level || "L?")} now · score ${numText(score)} · ${numText(domainLoad.etu || 0)} ETU · ${htmlText(forecastability.reason)}</span></span><strong>—</strong><span>Not forecastable</span></div>`;
    }
    const domainConfidence = Math.min(Number(confidence?.index || 0.4), domainConfidenceIndexSafe(x?.confidence)) * Number(forecastability.dampener || 1);
    const baseProb = predictionProbabilitySafe(score, nextThreshold, effectiveVelocity, domainConfidence, velocity?.volatility);
    const etuProgress = Math.max(0, Math.min(1, Number(domainLoad.etu || 0) / Math.max(1, requiredEtu)));
    const prob = Math.round(Math.max(1, Math.min(82, baseProb * (0.52 + etuProgress * 0.48))));
    const status = predictionBandStatusSafe(prob);
    const loadWindow = `${numText(domainLoad.etu || 0)} / ${requiredEtu} domain ETU`;
    return `<div class="context-row prediction-row ${status.cls}"><span>${htmlText(label)}<br><span class="muted">${htmlText(x?.level || "L?")} now · score ${numText(score)} · next band around ${numText(nextThreshold)} · ${loadWindow}</span></span><strong>${prob}%</strong><span>${htmlText(status.label)}</span></div>`;
  }).join("");
}
...
function predictionShortDateSafe(value) {
...
function predictionRollingEtuSafe(rows, days) {
  const dated = (rows || []).filter(r => Number.isFinite(Number(r.dateValue))).slice().sort((a,b)=>Number(a.dateValue)-Number(b.dateValue));
  if (!dated.length) return 0;
  const end = Number(dated[dated.length - 1].dateValue);
  const start = end - Math.max(1, Number(days || 7)) * 86400000;
  return dated.filter(r => Number(r.dateValue) >= start).reduce((sum, r) => sum + Math.max(0, Number(r.etu || 0)), 0);
}
function predictionEtuBandSafe(etu) {
  const v = Number(etu || 0);
  if (v >= 3.6) return {label:"High development", cls:"good"};
  if (v >= 2.2) return {label:"Development", cls:"watch"};
  if (v >= 1.0) return {label:"Light / maintenance", cls:"watch"};
...
function predictionCumulativeEtuSafe(rows) {
  ...
  const values = recent.map(r => {
    cumulative += Number(r?.etu || 0);
    return cumulative;
  });
  const refs = [
    { label: "Junior ref", value: 120 },
    { label: "Club ref", value: 360 },
    { label: "Senior ref", value: 700 }
  ];
  const maxRef = Math.max(...refs.map(r => r.value));
  const maxValue = Math.max(maxRef, ...values, 1);
  const w = 320;
  const h = 150;
  const left = 42;
  const right = 14;
  ...
  const xFor = i => left + (recent.length <= 1 ? 0 : (i / (recent.length - 1)) * plotW);
  const yFor = v => top + plotH - (Math.max(0, Math.min(maxValue, Number(v || 0))) / maxValue) * plotH;
  const points = values.map((v, i) => `${xFor(i).toFixed(1)},${yFor(v).toFixed(1)}`).join(" ");
  const refLines = refs.map(r => {
    const y = yFor(r.value);
    return `<g class="prediction-etu-ref"><line x1="${left}" y1="${y.toFixed(1)}" x2="${w-right}" y2="${y.toFixed(1)}"></line><text x="${left+4}" y="${Math.max(10, y-3).toFixed(1)}">${htmlText(r.label)} · ${r.value} ETU</text></g>`;
  }).join("");
...
function predictionEtuQualityMixSafe(rows) {
  ...
  const buckets = [
    {key:"micro", label:"Micro / recovery", min:0, max:1},
    {key:"light", label:"Light / maintenance", min:1, max:2.2},
    {key:"dev", label:"Development", min:2.2, max:3.6},
    {key:"high", label:"High development", min:3.6, max:999}
  ];
  const total = Math.max(0.01, list.reduce((sum, r) => sum + Math.max(0, Number(r.etu || 0)), 0));
  return `<div class="prediction-etu-mix">${buckets.map(b => {
    const etu = list.filter(r => Number(r.etu || 0) >= b.min && Number(r.etu || 0) < b.max).reduce((sum, r) => sum + Math.max(0, Number(r.etu || 0)), 0);
    const pct = Math.max(0, Math.min(100, etu / total * 100));
    return `<div class="context-row"><span>${htmlText(b.label)}</span><div class="prediction-etu-bar-track"><i style="width:${pct.toFixed(1)}%"></i></div><strong>${Math.round(pct)}%</strong></div>`;
  }).join("")}</div>`;
}
function predictionEtuComponentBreakdownSafe(load) {
  const c = load?.componentAverages || {};
  const items = [
    {label:"Duration", value:Number(c.duration || 0), detail:"diminishing returns after ~90m"},
    {label:"Diversity", value:Number(c.diversity || 1), detail:"more distinct routines"},
    {label:"Routine density", value:Number(c.density || 1), detail:"session structure"},
    {label:"Pressure", value:Number(c.pressure || 1), detail:"match/tactical exposure"},
    {label:"Transfer", value:Number(c.transfer || 1), detail:"break-building bridge work"},
    {label:"Adaptive", value:Number(c.adaptive || 1), detail:"smart/recommended work"},
    {label:"Challenge", value:Number(c.challenge || 1), detail:"productive target band"},
    {label:"Quality", value:Number(c.quality || 1), detail:"subjective quality"},
    {label:"Fatigue", value:Number(c.fatigue || 1), detail:"fatigue penalty"}
  ];
  const max = Math.max(1.6, ...items.map(x => Number(x.value || 0)));
  return `<div class="prediction-etu-component-list">${items.map(x => {
    const pct = Math.max(4, Math.min(100, Number(x.value || 0) / max * 100));
    const modifier = Number(x.value || 1) >= 1.04 ? "+" : Number(x.value || 1) <= 0.96 ? "−" : "=";
    return `<div class="prediction-domain-bar"><span>${htmlText(x.label)}</span><div class="prediction-domain-track"><b style="width:${pct.toFixed(1)}%"></b></div><strong>${modifier}${numText(x.value)}</strong><small>${htmlText(x.detail)}</small></div>`;
  }).join("")}</div>`;
}


function predictionRollingEtuWindowSafe(rows, days) {
  const dated = (rows || []).filter(r => Number.isFinite(Number(r.dateValue))).slice().sort((a,b)=>Number(a.dateValue)-Number(b.dateValue));
  if (!dated.length) return { etu:0, sessions:0, days:Number(days || 0), start:null, end:null };
  const end = Number(dated[dated.length - 1].dateValue);
  const span = Math.max(1, Number(days || 7));
  const start = end - span * 86400000;
  const selected = dated.filter(r => Number(r.dateValue) >= start);
  return {
    etu: selected.reduce((sum, r) => sum + Math.max(0, Number(r.etu || r.effectiveEtu || 0)), 0),
...
function predictionLoadManagementSafe(rows, load) {
  const list = Array.isArray(rows) ? rows : [];
  const w7 = predictionRollingEtuWindowSafe(list, 7);
  ...
  const variance = recent.length > 1 ? recent.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) / recent.length : 0;
...
function predictionLoadBarSafe(label, value, max, detail) {
  ...
  return `<div class="prediction-domain-bar"><span>${htmlText(label)}</span><div class="prediction-domain-track"><b style="width:${pct.toFixed(1)}%"></b></div><strong>${numText(value)}</strong><small>${htmlText(detail || "ETU")}</small></div>`;
}
function renderPredictionEtuLoadManagementSafe(load) {
  try {
...
function predictionRecoveryReadinessSafe(load) {
  const rows = Array.isArray(load?.rows) ? load.rows : [];
  const mgmt = predictionLoadManagementSafe(rows, load || {});
  const latest = rows.length ? rows[rows.length - 1] : null;
  const recent = rows.slice(-5);
  const latestEtu = Number(latest?.effectiveEtu || latest?.etu || 0);
  const recentFatigueVals = recent.map(r => Number(r.avgFatigue)).filter(Number.isFinite);
  const recentQualityVals = recent.map(r => Number(r.avgQuality)).filter(Number.isFinite);
  const recentFatigue = recentFatigueVals.length ? avg(recentFatigueVals) : null;
  const recentQuality = recentQualityVals.length ? avg(recentQualityVals) : null;
  const acute = Number(mgmt.acuteRatio || 0);
  const consistency = Number(mgmt.consistency || 0);
  const dated = rows.map(r => Number(r.dateValue)).filter(Number.isFinite).sort((a,b)=>a-b);
  const lastDate = dated.length ? dated[dated.length - 1] : null;
  const gapDays = Number.isFinite(lastDate) ? Math.max(0, (Date.now() - lastDate) / 86400000) : null;
  ...
  else if (latestEtu >= 3.2) readiness -= 8;
  else if (latestEtu > 0 && latestEtu < 1.2) readiness += 4;
  if (Number.isFinite(recentFatigue)) readiness -= Math.max(0, recentFatigue - 2.2) * 9;
  if (Number.isFinite(recentQuality)) readiness += (recentQuality - 3.0) * 5;
  if (consistency >= 70) readiness += 6;
  else if (consistency < 45) readiness -= 6;
  if (Number.isFinite(gapDays)) {
    if (gapDays >= 3 && gapDays <= 6) readiness += 8;
    else if (gapDays > 10) readiness -= 6;
  }
  readiness = Math.round(Math.max(5, Math.min(95, readiness)));

  let nextType = "consolidation";
  let label = "Consolidate";
  let guidance = "Use stable routines, moderate difficulty and stop before quality drops.";
  let cls = "watch";
  if (readiness >= 74 && mgmt.status.cls !== "risk") {
    nextType = "acquisition";
    label = "Ready to build";
    guidance = "A normal acquisition or transfer session is acceptable. Keep pressure work controlled rather than maximal.";
    cls = "good";
  } else if (readiness >= 58) {
    nextType = "transfer";
    label = "Controlled transfer";
    guidance = "Use one focused transfer block and avoid stacking several benchmark routines.";
    cls = "good";
  } else if (readiness >= 42) {
    nextType = "consolidation";
    label = "Consolidate / light";
    guidance = "Choose easy-to-moderate cueing, cue-ball control or confidence routines. Avoid hard benchmark attempts.";
    cls = "watch";
  } else if (readiness >= 28) {
    nextType = "recovery";
    label = "Recovery block";
    guidance = "15–30 minutes only: low-pressure cueing, simple positional work, or very light safety touch.";
    cls = "watch";
  } else {
    nextType = "rest";
    label = "Rest preferred";
    guidance = "Skip table work or do only visualization / review. Do not chase ETU today.";
    cls = "risk";
  }

  const blocks = {
    acquisition: "Main technical gap, fresh concentration, productive difficulty.",
    transfer: "Bridge isolated skills into break-building or frame-like execution.",
    pressure: "One-attempt or consequence work only when readiness is good.",
    consolidation: "Stabilize recent gains with familiar routines and moderate targets.",
    recovery: "Low-pressure cueing and confidence recovery; no benchmark chasing.",
    rest: "No physical table load; review notes, visualize, or let adaptation occur."
  };
  const reasons = [];
  ...
  return {readiness, nextType, label, guidance, cls, mgmt, latestEtu, recentFatigue, recentQuality, gapDays, blocks, reasons};
}
function predictionSessionTypeBarSafe(name, active, detail) {
  return `<div class="prediction-domain-bar ${active ? "good" : ""}"><span>${htmlText(name)}</span><div class="prediction-domain-track"><b style="width:${active ? 100 : 28}%"></b></div><strong>${active ? "Next" : ""}</strong><small>${htmlText(detail || "")}</small></div>`;
}
function renderPredictionRecoveryReadinessSafe(load) {
  try {
    const r = predictionRecoveryReadinessSafe(load || {});
    const pct = Math.max(3, Math.min(100, Number(r.readiness || 0)));
    const types = [
...
function renderPredictionEtuVisualsSafe(load) {
  try {
    const rows = Array.isArray(load?.rows) ? load.rows : [];
    const rolling7 = predictionRollingEtuSafe(rows, 7);
